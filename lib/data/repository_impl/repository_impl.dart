...
class RepositoryImpl implements Repository{
  ...
  @override
  Future<Either<Failure, LoginOrRegisterOrResetPasswordModel>> login(LoginRequest loginRequest) async {
    if(await _networkInfo.isConnected){
      try{
        LoginOrRegisterOrResetPasswordResponse response = await _remoteDataSource.login(loginRequest);
  ...
  @override
  Future<Either<Failure, LoginOrRegisterOrResetPasswordModel>> register(RegisterRequest registerRequest) async {
    if(await _networkInfo.isConnected){
      try{
        LoginOrRegisterOrResetPasswordResponse response = await _remoteDataSource.register(registerRequest);
        if(response.status == true) {
          return Right(response.toDomain());
        }else{
          return Left(Failure(response.status.orFalse(), response.message.orEmpty()));
  ...
  @override
  Future<Either<Failure, LoginOrRegisterOrResetPasswordModel>> resetPassword(ResetPasswordRequest resetPasswordRequest) async {
    if(await _networkInfo.isConnected){
      try{
        LoginOrRegisterOrResetPasswordResponse response = await _remoteDataSource.resetPassword(resetPasswordRequest);
        if(response.status == true) {
          return Right(response.toDomain());
        }else{
          return Left(Failure(response.status.orFalse(), response.message.orEmpty()));
  ...
  @override
  Future<Either<Failure, CategoryAllDataModel>> getCategoryProducts(int categoryId) async {
    try{
      final response = await _localDataSource.getCategoryProductsResponse(categoryId);
      return Right(response.toDomain());
    }catch(cacheError){
      if(await _networkInfo.isConnected){
        try{
          CategoryAllDataResponse response = await _remoteDataSource.getCategoryProducts(categoryId);
          if(response.status == true) {
            await _localDataSource.saveCategoryProductsToCache(response,categoryId);
            return Right(response.toDomain());
          }else{
            return Left(Failure(response.status.orFalse(), response.message.orEmpty()));
          }
